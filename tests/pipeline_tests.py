...
if __name__ == "__main__":
    ...
    test_path = "./tests/output/english_labeled_data_with_scores.csv"
    ...
    model_output_path = "./scripts/output/english_data_merged.csv"
    # model_output_path = "./scripts/output/Hebrew_data_merged.csv"
    ...
    test_df["Summary"] = clean_summary(test_df["Summary"])
    model_output_df["Summary"] = clean_summary(model_output_df["Summary"])
    ...
    test_grouped = test_df.groupby(['Summary'])['sent_score'].mean().reset_index()
    # test_grouped = test_df.groupby(['Summary'])['sent_score_mean'].mean().reset_index().rename(columns={'sent_score_mean': 'score_test'})
    # model_output_grouped = model_output_df.groupby(['Summary'])[["emd_score", "stance_match"]].mean().reset_index()
    model_output_grouped = model_output_df.groupby(['Summary'])["emd_score"].mean().reset_index()
